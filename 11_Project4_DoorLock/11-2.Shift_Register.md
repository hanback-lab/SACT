...
22. 핀 설정 후 창을 닫고, Processing > Start Compilation 메뉴를 선택하여 컴파일을 진행한다. 
...
23. SACT 장비를 준비한다. USB 케이블과 파워 케이블을 연결하고, 전원 스위치를 눌러 장비에 전원을 인가시킨다. 

24. Quartus 소프트웨어에서 Tool > Programmer 메뉴를 선택한다.

25. Programmer창의 Hardware Setup이 USB Blaster가 연결되어 있는지 확인하고, Start 버튼을 눌러 프로그래밍 하고 장비에서 동작을 확인한다. 


26. DIN의 값을 설정하고, CLK에 대한 버튼 스위치를 눌러 데이터가 전달 되는 것을 확인해 보자. 
