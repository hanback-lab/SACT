...
설계하려는 장치는 4bit A와 B 데이터의 나눗셈기이다. 


4비트 A와 B의 나눗셈을 뺄셈으로 바꾸어 계산해 보면 아래와 같다. 

<BR>
A의 데이터는 1101이고, B의 데이터는 0011이라고 가정한다. 
...
- (1)<BR>
    - ( [0 0 0 A3] - B )를 수행한다. 
    - 빌림 수(Br)가 발생하여 다음으로 넘어간다.  <BR>
    - 첫째 자리의 값은 0이다.<BR>
    - (A3와 B를 비교해서 B > A 이면 다음으로 넘어간다로 해석할 수도 있다. )
    <BR><BR>

- (2)<BR>
    - ( [0 0 A3 A2] - B )를 수행한다. 
    - 빌림 수가 발생하지 않았기 때문에 두번째 자리의 값은 1이다. 
    <BR><BR>

- (3)<BR>
    - ( [0 (A3-B2) (A2-B1) A1] - B )를 수행한다. 
    - 빌림 수(Br)가 발생하여 다음으로 넘어간다.  <BR>
    - 셋째 자리의 값은 0이다. <BR>

- (4) <BR>
    - ( [(A3-B2) (A2-B1) A1 A0] - B )를 수행한다. 
    - 빌림 수(Br)가 발생하여 다음으로 넘어간다.  <BR>
    - 넷째 자리의 값은 0이다. <BR>

- (5) <BR>
    - 따라서 ( [1 1 0 1]  /  [0 0 1 1] )의 결과는 [0 1 0 0] 이다. <BR>
...
9. 다음 그림과 같이 input과 output의 이름을 수정하고, 심볼을 더 추가해 심볼 사이를 wire로 연결하여 도면을 완성시킨다. 
    ...
    - P_ADD_SUB의 M에 VCC를 연결하여 감산기로 동작하도록 설정하였다. 
...
17. 시뮬레이션을 하기 전에, Simulation > Simulation Setting 메뉴를 선택하여, 나타나는 창에서 -novopt 부분을 지운다. 
    ...
    <img src="./pds/cnt10_14a.png" alt="traffic" style="width: 70%;"><br><br>

    >이 -novopt를 제거하지 않으면, 시뮬레이션을 진행했을 때 아래와 같은 메시지가 나타난다. 

    <img src="./pds/cnt10_14b.png" alt="traffic" style="width: 70%;"><br><br>
...
18. Simulation > Run Functional Simulation 메뉴를 선택하여 시뮬레이션을 진행한다. 
    ...
    <img src="./pds/div05.png" alt="traffic" style="width: 100%;"><br><br><br>
...
24. 핀 설정 후 창을 닫고, Processing > Start Compilation 메뉴를 선택하여 컴파일을 진행한다. 
